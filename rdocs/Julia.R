...
banco1<-banco%>%
  select(CAD, Altitude = ALTITUDE, `Areia Fina`= AREIA_FINA,`Areia Grossa`=AREIA_GROS, Argila=ARGILA, CC,  Densidade=DENSIDADE, Profundidade = PROFUND, PMP, Domínio = dominios_n, Perfil=PERFIL,  Silte=SILTE)
...
banco1$Domínio<- factor(banco1$Domínio)
# banco1$Mesorregião <- factor(banco1$Mesorregião)
# banco1$Região <- factor(banco1$Região)
...
############### modelo inicial ###############
# 
 reg1 <- lm(data = banco1 , log(CAD) ~ log(Altitude) + log(Profundidade)  + log(Silte) + log(Argila) +log(`Areia Grossa`) + log(`Areia Fina`) + log(Densidade))
 summary(reg1) 
# 
 plot(reg1$fitted.values,banco1$CAD)
# 
 x11()
 par(mfrow=c(2,2))
 plot(reg1)
 ...
 reg2 <- lm(data = banco1 , CAD ~ log(Altitude) + log(Profundidade)  + log(Silte) + log(Argila) +log(`Areia Grossa`) + log(`Areia Fina`) + log(Densidade))
 summary(reg2)
 x11()
 par(mfrow=c(2,2))
 plot(reg2)
...
Sertões<- banco1%>%
  filter(str_sub(Domínio,) == "Sertões") %>%
  select(CAD,Altitude , `Areia Fina`,`Areia Grossa`, Argila, CC,  Densidade, Profundidade,  Silte)

modelo <- lm(data = Sertões, CAD ~ Altitude +`Areia Fina`+`Areia Grossa`+ Argila+ CC+ Densidade+ Profundidade+  Silte)
summary(modelo)
coef(modelo)
outlierTest(modelo)
...
modelo_inicial <- lm(modelo, data = Sertões)
modelo_stepwise <- step(modelo_inicial, direction = "both")

modmin<-lm(CAD ~ 1, data=Sertões)
step(modmin, direction='forward', scope=( ~ CC + Altitude + Profundidade  + Silte + Argila +`Areia Grossa` + `Areia Fina` + Densidade))

modmax<-lm(CAD ~ CC + Altitude + Profundidade  + Silte + Argila +`Areia Grossa` + `Areia Fina` + Densidade, data=Sertões)
step(modmax, direction = 'backward')


# Modelo stepwise encontrado

modelo <- lm(data = Sertões, CAD ~ Altitude + `Areia Fina` + `Areia Grossa` + CC + Densidade + 
               Profundidade + Silte)
summary(modelo)

plot(modelo$fitted.values,Sertões$CAD)
